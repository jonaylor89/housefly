...
export const resolvers = {
  Query: {
    challenges: (_: any, args: { category?: string; first?: number; after?: string }) => {
      let filteredChallenges = [...challenges];
      
      // Filter by category if provided
      if (args.category) {
        filteredChallenges = filteredChallenges.filter(c => c.category === args.category);
      }
      ...
      if (args.after) {
        const decodedCursor = Buffer.from(args.after, 'base64').toString('ascii');
        afterIndex = filteredChallenges.findIndex(c => c.id === decodedCursor);
      }
  ...
  User: {
    bookmarks: (parent: { bookmarks: string[] }) => {
      return parent.bookmarks.map(id => challenges.find(c => c.id === id)).filter(Boolean);
    },
    completedChallenges: (parent: { completedChallenges: string[] }) => {
      return parent.completedChallenges.map(id => challenges.find(c => c.id === id)).filter(Boolean);
    },
  },
  Mutation: {
    ...
    bookmark: (_: any, { challengeId }: { challengeId: string }, context: Context) => {
    ...
    unbookmark: (_: any, { challengeId }: { challengeId: string }, context: Context) => {
      if (!context.user) throw new Error('Not authenticated');
    ...
    completeChallenge: (_: any, { challengeId }: { challengeId: string }, context: Context) => {
      if (!context.user) throw new Error('Not authenticated');
    ...
    addComment: (_: any, { challengeId, content }: { challengeId: string; content: string }, context: Context) => {
      if (!context.user) throw new Error('Not authenticated');
    ...
    addReply: (_: any, { commentId, content }: { commentId: string; content: string }, context: Context) => {
      if (!context.user) throw new Error('Not authenticated');
