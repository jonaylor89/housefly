...
在本章中，你将应对"CryptoDefend Exchange"——一个模拟的加密货币交易平台，它不希望自己的数据被轻易访问。就像许多金融网站一样，CryptoDefend实施了各种防御措施，防止自动收集价格数据、交易量和市场趋势。
...
验证码呈现的是对人类来说容易但对机器人来说困难的任务。现代验证码如 reCAPTCHA v3 甚至会在后台隐形运行，分析用户行为：
...
第11章中我们的CryptoDefend交易所呈现了在收集金融数据时可能遇到的真实挑战。你需要应对：
