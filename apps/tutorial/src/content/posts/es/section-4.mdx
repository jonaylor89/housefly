...
*Encuentra una solución de referencia demostrando el scraping directo de API en el directorio `exercises/chapter-07/solution/src/`.*

### 2. Formularios y Autenticación (Capítulo 8)

A menudo, los datos valiosos están detrás de una pantalla de inicio de sesión o requieren enviar formularios complejos. Por ejemplo, la plataforma de reservas de viajes en el Capítulo 8 exige autenticación para acceder a la funcionalidad principal. Para buscar destinos (usando autocompletar), seleccionar fechas de viaje (interactuando con un widget de calendario), aplicar filtros y ver resultados (incluyendo listados premium solo disponibles para usuarios que hayan iniciado sesión), primero necesitas automatizar el proceso de inicio de sesión. Esto involucra manejar formularios, gestionar cookies de sesión (incluyendo posibles timeouts que requieren re-autenticación y protección CSRF), y finalmente controlar el navegador para realizar acciones como un usuario real.
...
*   **Acceso a Contenido Protegido:** Una vez autenticado, puedes navegar y hacer scraping de páginas o datos solo disponibles para usuarios que hayan iniciado sesión.

**El Desafío (Capítulo 8):** Este capítulo involucra un proceso de múltiples pasos: iniciar sesión en un sitio, navegar a una página de búsqueda, llenar un formulario complejo de múltiples partes con filtros, extraer los resultados (incluyendo contenido premium solo visible cuando se ha iniciado sesión), e incluso guardar la búsqueda en un dashboard de usuario.
